...
function buildRules(): Record<string, P> {
  ...
  rule("year_digits", () =>
    str("19").or(str("20")).then(match("\\d").repeat(2, 2), ref(rules, "digits").absent()),
  );
  rule("month_digits", () => match("\\d").repeat(2, 2));
  rule("day_digits", () => match("\\d").repeat(2, 2));
  rule("originator", () =>
    ref(rules, "organization")
      .as("publisher")
      .then(
        ref(rules, "space?")
  ...
  rule("comma_month_year", () =>
    ref(rules, "comma").then(ref(rules, "words").as("month"), str(" "), ref(rules, "year_digits").as("year")),
  );
  rule("year_month", () =>
    ref(rules, "year_digits").then(ref(rules, "dash"), ref(rules, "month_digits")),
  );
  ...
  rule("identifier", () =>
    ref(rules, "amendment_identifier")
  ...
  rule("full_number", () =>
    ref(rules, "number_only")
      .then(ref(rules, "part_number"), ref(rules, "subpart_number"), ref(rules, "named_suffix").maybe())
      .or(
        ref(rules, "number_only").then(ref(rules, "part_number"), ref(rules, "named_suffix").maybe()),
      )
  ...
  rule("edition_portion", () =>
    str(", ")
      .or(ref(rules, "space"))
      .then(
        ref(rules, "edition_number").maybe(),
        ref(rules, "space?"),
        ref(rules, "edition_text"),
        ref(rules, "space?"),
        ref(rules, "year_digits").as("year"),
      )
      .as("edition_format"),
  );
  ...
  rule("stage_iteration", () =>
    match("\\d").repeat(1).then(str("."), match("\\d").repeat(1)).or(match("\\d").repeat(1)).as("iteration"),
  );

  rule("stage_abbr", () => str("WD").or(str("CD")).as("stage"));

  rule("draft_stage", () =>
    ref(rules, "space").then(ref(rules, "stage_iteration").maybe(), ref(rules, "stage_abbr")),
  );
  ...
  rule("language_with_space", () =>
    ref(rules, "space")
      .then(ref(rules, "lparen"), ref(rules, "language_code"), ref(rules, "rparen"))
      .then(str("").as("space_before_lang")),
  );

  rule("language_without_space", () =>
    ref(rules, "lparen").then(ref(rules, "language_code"), ref(rules, "rparen")),
  );

  rule("language_portion", () =>
    ref(rules, "language_with_space").or(ref(rules, "language_without_space")),
  );
  ...
  rule("trailing_supplement_identifier", () =>
  ...
  rule("annex_letter_value", () =>
    match("[A-Z]").then(ref(rules, "dash").then(match("[A-Z]")).maybe()).as("annex_letter"),
  );
...
export const oimlGrammar: Grammar = {
  rules: buildRules(),
  root: "identifier",
};
