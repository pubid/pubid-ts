 ...
 */
export function checkCase(testCase, impl) {
    ...
    try {
        ...
        return [`${label} raised ${error.name}`];
    }
    const mismatches = [];
    ...
    if (testCase.representations.urn !== undefined) {
        ...
        if (urn !== testCase.representations.urn)
            mismatches.push(`${label} urn`);
    }
...
export function checkNegatives(payloads, impl) {
    const failures = [];
    let reclassified = 0;
...
export function runFlavor(flavor, payloads, impl, pending) {
    const report = {
        flavor,
        outcome: "pass",
        cases: 0,
    ...
    for (const testCase of payloads.cases) {
        ...
        if (pending.has(testCase.id)) {
            ...
            if (mismatches.length === 0)
                report.pendingSatisfied.push(testCase.id);
        }
        else {
            report.cases += 1;
...
export function runCorpus(corpus, implementations, pending) {
    const flavors = [];
    let gate = "pass";
