...
test("open-ended parsing beyond the corpus (the point of the waves)", () => {
    assert.equal(grammarImplementation("doi").parse("doi:10.9999/nist.SP.260-220").toHuman(), "doi:10.9999/nist.SP.260-220");
    assert.equal(grammarImplementation("iana").parse("IANA dns-sec-alg-numbers/1").toUrn(), "urn:iana:dns-sec-alg-numbers:1");
    assert.equal(grammarImplementation("un").parse("S/2023/DOC/9").toHuman(), "S/2023/DOC/9");
    assert.equal(grammarImplementation("xsf").parse("XEP 9999").toUrn(), "urn:xsf:xep:9999");
});
